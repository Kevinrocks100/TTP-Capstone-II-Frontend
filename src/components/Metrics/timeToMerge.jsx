...
function TimeToMerge({ repo, owner, name }) {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API_URL}/api/calculate_merge_duration/${owner}/${name}`)
            .then(response => {
    ...
    const processResponseData = rawData => {
        ...
        const mergeDurations = rawData.mergeDurations;
        const averageDuration = rawData.averageDuration;
